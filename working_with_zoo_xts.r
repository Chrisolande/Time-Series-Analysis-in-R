...
# Annual aggregation
USgas_zoo_yr <- aggregate(USgas_zoo, by = year, FUN = sum)
...
ggplot(brent_df, aes(x = date)) +
  geom_ribbon(
    aes(ymin = pmin(price, ma_12), ymax = pmax(price, ma_12)),
    alpha = 0.2,
    fill = "steelblue"
  ) +
